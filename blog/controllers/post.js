...
exports.create = async (req, res, next) => {
    ...
    let post;
    try {
        post = models.Posts.build( { title, body, authorId } );
        // Saves only the fields title and body into the DDBB
...
const createPostAttachment = async (req, post) => {
    ...
    const url = `${req.protocol}://${req.get('host')}/post/${post.postId}/attachment`;
    // Create the new attachment into the data base.
    const attachment = await models.Attachment.create( { mime: req.file.mimetype, image, url } );
